/**
 * Solana Wallet Manager Component
 */

import { useState } from 'react';
import { Copy, Plus, Wallet, Trash2, RefreshCw } from 'lucide-react';
...
import { useSolanaWallets, useSolanaBalance } from '@/hooks/useSolana';
import type { SolanaNetwork } from '@/lib/solana/types';

export function SolanaWalletManager() {
  const { wallets, activeWallet, create, import: importWallet, setActive, remove } = useSolanaWallets();
  ...
  const handleImport = () => {
    ...
    try {
      const secretArray = importSecret.split(',').map(Number);
      importWallet(newWalletName, secretArray, network, false);
      setNewWalletName('');
      setImportSecret('');
    } catch {
      // Handle error
        ...
        <Dialog>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 pt-4">
              ...
              <div className="space-y-2">
                <Label>Secret Key bytes (for import, comma-separated)</Label>
                <Input
                  value={importSecret}
            ...
            <DialogContent>
              ...
              <div className="space-y-4 pt-4">
                <p className="text-sm text-muted-foreground">
                  This is your secret key (byte array). Save it securely - it won't be shown again!
                </p>
                <div className="flex items-center gap-2">
                  <Input value={showSecret.join(',')} readOnly type="password" className="font-mono text-xs" />
                  <Button
                    size="icon"
                    variant="outline"
                    onClick={() => navigator.clipboard.writeText(showSecret.join(','))}
                  >
...
function WalletCard({
  ...
  const { balance, isLoading, refresh } = useSolanaBalance(wallet.publicKey, wallet.network as SolanaNetwork);
